...
def parser_ground_line(line):
    ...
    if "t2bbox" in task_name:
        gt_answer = (
            line["gt_answer"].strip().replace("<bbox>", "").replace("</bbox>", "")
        )
        gt_answer = [max(min(int(x) / 999, 1.0), 0.0) for x in gt_answer.split(",")]

        model_answer = (
            line["model_answer"].strip().replace("<bbox>", "").replace("</bbox>", "")
        )
        try:
            model_answer = [
                max(min(int(x) / 999, 1.0), 0.0) for x in model_answer.split(",")
            ]
        ...
        model_answer = (
            line["model_answer"].strip().replace("<ocr>", "").replace("</ocr>", "")
        )
...
def llm_benchmark_eval(
    metric_names=["ContainAccuracy"], result_path="", save_each_eval=True
):
...
def llm_text_localization_eval(
    metric_names=["BLEU1", "BLEU2", "BLEU3", "BLEU4"],
    result_path="",
    save_each_eval=True,
):
    if not Path(result_path).exists():
...
def llm_textcaps_textvqa_eval(
    result_path, dataset="TextVQA", split="test", meta_dir=""
):
    if dataset == "TextVQA":
        ...
        if not os.path.exists(question_ids_path):
            qa_path = os.path.join(
                meta_dir, dataset, "TextVQA_0.5.1_" + split + ".json"
            )
