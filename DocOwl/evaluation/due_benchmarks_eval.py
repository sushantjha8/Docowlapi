...
def eval_due(dataset_name, pred_path, gt_path):
    ...
    for metric in metrics:
        evaluator = DueEvaluator(
            reference=gts, answers=preds, ignore_case=True, metric=metric
        )
            ...
            if due_preds[i]["name"] == line["name"]:
                ...
                for anno in line["annotations"]:
                    img["annotations"].append(
                        {
                            "key": anno["key"],
                            "values": [{"value": random.choice(["0", "1"])}],
                        }
                    )
                new_due_preds.append(img)
                i -= 1
...
def llm_duebenchmark_eval(dataset_name, split, llm_pred_path, meta_dir):
    ...
    for i in range(len(meta_data)):
        ...
        if dataset_name in ["DeepForm", "KleisterCharity"]:
            cate_question = json.loads(meta_data[i]["ground_truth"])["gt_parses"][0][
                "question"
            ]
        ...
        # keep quesiton consistent with gt file is necessary for due eveluation
            ...
            preds[i]["question"] = json.loads(meta_data[i]["ground_truth"])[
                "gt_parses"
            ][0]["question"]
    ...
    for i in range(len(preds)):
        ...
        if (
            dataset_name not in ["DeepForm", "KleisterCharity"]
            or pred["answer"] != "None"
        ):
            img["annotations"].append(
                {"key": pred["question"], "values": [{"value": pred["answer"]}]}
            )
