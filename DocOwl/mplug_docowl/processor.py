...
def anchor_rank(anchors, anchors_areas, input_image_size, eps=1e-5):
    ...
    # image_size: (h, w)
    # xyxy
    input_image_bbox = torch.tensor(
        [0, 0, input_image_size[1], input_image_size[0]]
    ).unsqueeze(0)
    ...
    # y2
    boxes3[:, 3] = (
        input_image_size[0] / input_image_size[1] * anchors[:, 2]
    )  # 用于算分辨率无关的iou
...
class AnchorResize(torch.nn.Module):
    ...
    def forward(self, img, skip_resize=False):
        ...
        """
        selected_anchor = anchor_rank(
            self.anchors, self.anchor_areas, (img.size[1], img.size[0])
        )
...
class DocProcessor:
    def __init__(
        ...
        self.old_resizer = transforms.Resize(
    ...
    def _process_image(self, images):
        ...
        for image in images:
            ...
            patch_position = torch.cat(
                [
                    repeat(
                        torch.arange(anchor[0]),
                        "num_h -> num_h num_w 1",
                        num_w=anchor[1],
                    ),
                    repeat(
                        torch.arange(anchor[1]),
                        "num_w -> num_h num_w 1",
                        num_h=anchor[0],
                    ),
                ],
            ...
            patch_position = rearrange(
                patch_position, "num_h num_w p-> (num_h num_w) p", p=2
            )  # num_patch, (ph,pw)

            if self.add_global_img:
                patch_position = torch.cat(
                    [torch.ones(1, 2).long() * self.anchor_max, patch_position], dim=0
                )
