...
from mplug_docowl.train.llama_flash_attn_monkey_patch import (
...
def docowl_text_preprocess_v1(
    source, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False
) -> Dict:
    """
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        while True:
            ...
            query = source["messages"][0][
                "content"
            ]  # e.g. <|image|>what is the contact person name mentioned in letter?."
            ...
            image, patch_positions, processed_query = self.doc_image_processor(
                images=image_path, query=query
            )
...
def train():
    ...
    if training_args.tune_vision2text and training_args.bits in [4, 8]:
        model.get_model().vision2text.to(
            dtype=compute_dtype, device=training_args.device
        )
    else:
    ...
    model.config.tune_vision2text = (
        model_args.tune_vision2text
    ) = training_args.tune_vision2text
