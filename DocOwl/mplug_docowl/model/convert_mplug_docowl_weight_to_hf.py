...
llama_s2dense = {
    ...
    65: 22016,
    70: 28672,
}  # should be (2/3)*4*d, but it isn't exaclty that
...
def write_model(
    ...
    base = 10000.0
    inv_freq = 1.0 / (
        base ** (torch.arange(0, hidden_per_head, 2).float() / hidden_per_head)
    )
    ...
    def permute(w, skip_permute=skip_permute):
        if skip_permute:
            return w
        return (
            w.view(n_heads, n_hidden // n_heads // 2, 2, n_hidden)
            .transpose(1, 2)
    ...
    if num_shards == 1:
        ...
        if os.path.exists(os.path.join(input_base_path, "release")):
            filename = os.path.join(
                input_base_path, "release", "mp_rank_00", "model_optim_rng.pt"
            )
        elif input_base_path.split("/")[-1].startswith("iter_"):
            iteration = eval(
                input_base_path.split("/")[-1].replace("iter_", "").lstrip("0")
            )
            ...
            tracker_filename = os.path.join(
                input_base_path, "latest_checkpointed_iteration.txt"
            )
        ...
        for i in range(num_shards):
            if os.path.exists(os.path.join(input_base_path, "release")):
                filename = os.path.join(
                    input_base_path, "release", f"mp_rank_{i:02d}", "model_optim_rng.pt"
                )
            else:
                tracker_filename = os.path.join(
                    input_base_path, "latest_checkpointed_iteration.txt"
                ...
                iteration = "iter_{:07d}".format(int(metastring))
                filename = os.path.join(
                    input_base_path, iteration, f"mp_rank_{i:02d}", "model_optim_rng.pt"
                )
            if not os.path.exists(filename):
                filename = filename.replace("model_optim_rng.pt", "model_rng.pt")
    ...
    for layer_i in range(n_layers):
        ...
        if num_shards == 1:
            # Unsharded
            state_dict = {
                f"model.layers.{layer_i}.self_attn.q_proj.weight": loaded["encoder"][
                    f"layers.{layer_i}.self_attention.q_proj.weight"
                ],
                f"model.layers.{layer_i}.self_attn.k_proj.multiway.0.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.self_attention.k_proj.multiway.0.weight"],
                f"model.layers.{layer_i}.self_attn.v_proj.multiway.0.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.self_attention.v_proj.multiway.0.weight"],
                f"model.layers.{layer_i}.self_attn.k_proj.multiway.1.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.self_attention.k_proj.multiway.1.weight"],
                f"model.layers.{layer_i}.self_attn.v_proj.multiway.1.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.self_attention.v_proj.multiway.1.weight"],
                f"model.layers.{layer_i}.self_attn.o_proj.weight": loaded["encoder"][
                    f"layers.{layer_i}.self_attention.o_proj.weight"
                ],
                f"model.layers.{layer_i}.mlp.gate_proj.weight": loaded["encoder"][
                    f"layers.{layer_i}.mlp.gate_proj.weight"
                ],
                f"model.layers.{layer_i}.mlp.down_proj.weight": loaded["encoder"][
                    f"layers.{layer_i}.mlp.down_proj.weight"
                ],
                f"model.layers.{layer_i}.mlp.up_proj.weight": loaded["encoder"][
                    f"layers.{layer_i}.mlp.up_proj.weight"
                ],
                f"model.layers.{layer_i}.input_layernorm.multiway.0.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.input_layernorm.multiway.0.weight"],
                f"model.layers.{layer_i}.post_attention_layernorm.multiway.0.weight": loaded[
                    "encoder"
                ][
                    f"layers.{layer_i}.post_attention_layernorm.multiway.0.weight"
                ],
                f"model.layers.{layer_i}.input_layernorm.multiway.1.weight": loaded[
                    "encoder"
                ][f"layers.{layer_i}.input_layernorm.multiway.1.weight"],
                f"model.layers.{layer_i}.post_attention_layernorm.multiway.1.weight": loaded[
                    "encoder"
                ][
    ...
    if num_shards == 1:
        # Unsharded
        state_dict = {
            "model.embed_tokens.weight": loaded["embedding"]["word_embeddings"][
                "weight"
            ],
        ...
        state_dict = {
            "model.embed_tokens.weight": loaded[0]["embedding"]["word_embeddings"][
                "weight"
            ],
    ...
    state_dict.update(
        {
            "model.vision_model.embeddings.cls_token": loaded_all["vision_model"][
                "cls_token"
            ],
            ...
            "model.vision_model.post_layernorm.weight": loaded_all["vision_model"][
                "transformer"
    ...
    for v_layer_idx in range(24):
        state_dict.update(
            {
                f"model.vision_model.encoder.layers.{v_layer_idx}.input_layernorm.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.input_layernorm.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.mlp.fc1.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.mlp.fc1.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.mlp.fc2.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.mlp.fc2.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.post_attention_layernorm.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.post_attention_layernorm.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.self_attn.dense.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.self_attn.dense.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.self_attn.query_key_value.bias": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
                ...
                f"model.vision_model.encoder.layers.{v_layer_idx}.self_attn.query_key_value.weight": loaded_all[
                    "vision_model"
                ][
                    "transformer"
                ][
    ...
    state_dict.update(
        {
            ...
            "model.vision2text.visual_fc.bias": loaded_all["hreducer3"]["visual_fc"][
                "bias"
            ],
            "model.vision2text.visual_fc.weight": loaded_all["hreducer3"]["visual_fc"][
                "weight"
            ],
    ...
    state_dict.update(
        {
            f"model.vision2text.reducer_before.0.weight": loaded_all["hreducer3"][
                "reducer_before"
            ]["0.weight"],
            f"model.vision2text.reducer_before.0.bias": loaded_all["hreducer3"][
                "reducer_before"
            ]["0.bias"],
        }
    )
    ...
    state_dict.update(
        {
            f"model.vision2text.reducer.weight": loaded_all["hreducer3"]["reducer"][
                "weight"
            ],
            f"model.vision2text.reducer.bias": loaded_all["hreducer3"]["reducer"][
                "bias"
            ],
        }
    )
