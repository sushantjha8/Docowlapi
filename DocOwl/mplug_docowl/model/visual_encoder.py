...
class MplugDocOwlHReducerModel(PreTrainedModel):
    def __init__(self, config, language_hidden_size):
        ...
        self.conv_shape = (
            int(self.config.conv_shape.split("x")[0]),
            int(self.config.conv_shape.split("x")[1]),
        )  #
        ...
        self.reducer_before = torch.nn.Sequential(
            nn.Conv2d(
                ...
                self.conv_patch * self.config.hidden_size,
    ...
    def forward(self, encoder_hidden_states=None):
        ...
        encoder_hidden_states = rearrange(
            encoder_hidden_states, "B (H W) D -> B D H W", H=int(math.sqrt(L))
        )
        hidden_states = self.reducer_before(encoder_hidden_states)  # B 4D H W/4
        ## reduce seq length with a conv layer
        hidden_states = rearrange(
            hidden_states, "B (X D) H W -> B D H (W X)", X=self.conv_patch
        )  # B 4D H W/4 -> B D H W
        sequence_output = self.reducer(
            hidden_states
        )  # B,C,H,W -> B,C,H/conv_shape[0],W/(conv_shape[1])
        ...
        sequence_output = torch.cat(
            [sequence_output, self.vit_eos.repeat(B, 1, 1)], dim=1
        )
