...
from mplug_docowl.utils import build_logger, server_error_msg, pretty_print_semaphore

from mplug_docowl.constants import (
    IMAGE_TOKEN_INDEX,
    DEFAULT_IMAGE_TOKEN,
...
class ModelWorker:
    def __init__(
        ...
        self.token_num_each_img = (
            (self.resolution / 14)
            * (self.resolution / 14)
            / self.model.get_model().vision2text.conv_patch
        )
    ...
    def generate_stream(self, params):
        ...
        if images is not None and len(images) > 0 and self.is_multimodal:
            if len(images) > 0:
                ...
                images, patch_positions, prompt = self.doc_image_processor(
