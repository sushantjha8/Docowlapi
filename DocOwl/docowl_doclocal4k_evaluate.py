...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, choices=["text_grounding", "text_recognition"]
    )
    parser.add_argument(
        "--doclocal4k_dir", type=str, help="the directory path of DocLocal4K"
    )
    parser.add_argument(
    ...
    if os.path.exists(save_path):
        ...
        docowl = DocOwlInfer(
            ckpt_path=model_path, anchors="grid_9", add_global_img=False
        )
    ...
    if task == "text_recognition":
        ...
        llm_text_localization_eval(
            metric_names=["IOU@0.5"], result_path=pred_path, save_each_eval=True
        )
